...
The current complete capture and catalog are `2026-08-10-r2`. The fourteen current snapshots contain `22,052` accepted entries and `86` rejected records. The catalog contains `2,540` content addressed shards, represents every accepted entry, and assigns one exact blocker to every rejected record. `tests/committed-snapshot-counts.test.ts` pins the entry, rejection, and provenance-record counts for every historical and current committed snapshot.
