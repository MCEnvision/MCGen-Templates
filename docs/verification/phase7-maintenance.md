...
- `maintenance-pull-request` materializes a repository local, content addressed pull request manifest. It records source and catalog deltas, family, profile, and tuple blast radius, evidence invalidation and rebuild scope, coverage changes and blockers, exact verification commands, changed file areas, and an explicit no remote writes policy. The future GitHub App may consume this manifest, but this repository does not create branches or pull requests remotely.
...
node dist/cli.js phase7 pull-request --plan verification/phase7/maintenance-plan.json --monitor verification/phase7/monitor-run.json --invalidation verification/phase7/invalidation-plan.json --coverage verification/phase7/coverage-change.json --output verification/phase7/maintenance-pull-request.json
