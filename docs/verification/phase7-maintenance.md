...
The `phase7:monitor`, `phase7:quarantine`, `phase7:plan`, `phase7:audit`, and `phase7:recovery` npm scripts build the CLI before invoking the corresponding command. A scheduled run must pass an explicit `--generated-at` value for reproducible audit evidence.
