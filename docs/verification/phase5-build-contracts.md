...
The thin `phase 5 tuple verification` workflow validates the committed immutable evidence on pull requests, main updates, and the weekly scheduled audit. An explicit workflow dispatch runs the reviewed tuple commands again in bounded batches when a fresh build observation is required. `phase5-queue.ts` keeps changed boundaries, new tuples, invalidation, scheduled audits, and transient recovery as separate bounded queues with deterministic shard assignment and cancellation keys. Real tuple execution remains limited to profiles with reviewed exact commands and complete upstream evidence.
...
The reviewed boundaries are recorded in fifteen exact evidence files under [verification/phase5/evidence](../../verification/phase5/evidence). The generated [coverage summary](../../verification/phase5/coverage.json) reports fifteen verified tuples with no unresolved evidence. The [audit report](../../verification/phase5/audit.json) records fourteen reviewed profiles and fourteen reviewed descriptors with zero blocked records. The active catalog snapshot is `2026-08-11-r1`, and every nonverified catalog component has an explicit coverage blocker. The matrix and all reviewed execution inputs are rebound to this immutable snapshot, so a later catalog snapshot invalidates the evidence rather than silently reusing it.
