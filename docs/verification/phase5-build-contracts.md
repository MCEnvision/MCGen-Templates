...
The test suite also runs a real generated Bukkit smoke project through `javac` and `jar`, inspects its `plugin.yml` and entrypoint class, and repeats the package with fixed archive timestamps. This verifies the execution engine itself without promoting that synthetic smoke identity to a catalog profile. Production profile promotion still requires the exact upstream component, JDK, wrapper, and metadata evidence for the selected modern or historical boundary.
